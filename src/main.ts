...
async function main() {
    Object.entries(source).forEach(([docsName, data]) => {
        ...
        const docsIndex = Object.entries(path).reduce((prev, [locale, patterns]) => {
        ...
        Object.entries(docsIndex).forEach(([locale, allFiles]) => {
            console.log(`Generating search index for ${docsName} - ${locale} with ${allFiles.length} files.`);
            const searchIndex = generateSearchIndex(allFiles);
            const outputPath = join(__dirname, "..", "search-index", docsName, locale);
            fs.mkdirSync(outputPath, { recursive: true });
            fs.writeFileSync(join(outputPath, "search-index.json"), searchIndex, "utf-8");
        })
    });
}
